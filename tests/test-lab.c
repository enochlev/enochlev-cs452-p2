...
void test_oom(void)
{
    fprintf(stderr, "->Testing OOM\n");
    struct buddy_pool pool;
    size_t bytes = UINT64_C(1) << MIN_K;
    buddy_init(&pool, bytes);

    //add one large block then small block
    size_t ask = bytes - sizeof(struct avail);
    void *mem = buddy_malloc(&pool, ask);
...
void test_print(void)
{
    ...
    void *mem = buddy_malloc(&pool, 1);
    void *mem2 = buddy_malloc(&pool, 400);
    print_buddy_system(&pool);
    //Make sure correct kval was allocated
    buddy_free(&pool, mem);
    print_buddy_system(&pool);
...
void test_buddy_realloc(void)
{
    ...
    size_t size = UINT64_C(1) << kval;
    buddy_init(&pool, size);
    void *mem = buddy_malloc(&pool, 250000);
    print_buddy_system(&pool);
    //check if mem is not null
    assert(mem != NULL);

    void *mem2 = buddy_realloc(&pool, mem, 500000);
    print_buddy_system(&pool);
    //check if its full
    void *mem3 = buddy_malloc(&pool, 500001);
    print_buddy_system(&pool);
    check_buddy_pool_empty(&pool);
